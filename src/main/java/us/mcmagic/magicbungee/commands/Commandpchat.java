...
public class Commandpchat extends MagicCommand {
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(new ComponentBuilder("/pchat [Message]").color(ChatColor.RED).create());
            return;
        }
        Party party = MagicBungee.partyUtil.findPartyForPlayer(player.getUniqueId());
        if (party == null) {
        ...
        PacketChatMessage packet = new PacketChatMessage(player.getUniqueId(), player.getName(),
                "/pchat " + party.getLeader().getName() + " " + msg, ChatUtil.isParkChat(player.getServerName()),
                player.getServerName(), true);
        MagicBungee.socketConnection.sendMessage(packet);
        ChatUtil.logMessage(player.getUniqueId(), "/pchat " + party.getLeader().getName() + " " + msg);
    }
}
